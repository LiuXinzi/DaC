...
dxl_addparam_result_current = groupSyncRead_current.addParam(DXL1_ID)           #######################

if (dxl_addparam_result and dxl_addparam_result_current) != True:
    print("[ID:%03d] groupSyncRead addparam failed" % DXL1_ID)
    quit()

# # Add parameter storage for Dynamixel#2 present position value
# dxl_addparam_result = groupSyncRead.addParam(DXL2_ID)
# dxl_addparam_result_current = groupSyncRead_current.addParam(DXL2_ID)
# if (dxl_addparam_result and dxl_addparam_result_current) != True:
...
while 1:
    ...
    while 1:
        ...
        dxl_comm_result_current = groupSyncRead_current.txRxPacket()
        if dxl_comm_result_current != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result_current))
        ...
        dxl_getdata_result_current = groupSyncRead_current.isAvailable(DXL1_ID, ADDR_PRO_PRESENT_CURRENT, LEN_PRO_PRESENT_CURRENT)
        if dxl_getdata_result_current  != True:
            print("[ID:%03d] groupSyncRead_current getdata failed" % DXL1_ID)
            quit()
        ...
        # dxl_getdata_result_current = groupSyncRead_current.isAvailable(DXL2_ID, ADDR_PRO_PRESENT_CURRENT, LEN_PRO_PRESENT_CURRENT)
        # if dxl_getdata_result_current  != True:
        #     print("[ID:%03d] groupSyncRead_current getdata failed" % DXL2_ID)
        #     quit()

        # Get Dynamixel#1 present position value
        dxl1_present_position = groupSyncRead.getData(DXL1_ID, ADDR_PRO_PRESENT_POSITION, LEN_PRO_PRESENT_POSITION)
        dxl1_present_current = groupSyncRead_current.getData(DXL1_ID, ADDR_PRO_PRESENT_CURRENT, LEN_PRO_PRESENT_CURRENT)

        # # Get Dynamixel#2 present position value
        # dxl2_present_position = groupSyncRead.getData(DXL2_ID, ADDR_PRO_PRESENT_POSITION, LEN_PRO_PRESENT_POSITION)
        # dxl2_present_current = groupSyncRead_current.getData(DXL2_ID, ADDR_PRO_PRESENT_CURRENT, LEN_PRO_PRESENT_CURRENT)

        print("[ID:%03d] GoalPos:%03d  PresPos:%03d  PresCurr:%03d" % (DXL1_ID, dxl_goal_position[index], dxl1_present_position, dxl1_present_current))
