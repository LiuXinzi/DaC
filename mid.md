...
#### 通路
前六个关节角度：
1.通信过程： a机械臂树莓派通过连接自己的网口读取角度并通过静态网口（网线）传递给b机械臂树莓派 400HZ左右
2.控制过程： b机械臂树莓派接收数据到开始控制 1ms左右
#### 频率
本次测试主要使用三个时间作为频率数据
...
### 末端的控制通路（01）
   夹爪（第七个电机）电机型号为：DYNAMIXEL XH540-W150（文章里面是基于DYNAMIXEL XL430-W250）\
   预先将电机的运动范围写入，300和1200，开机后自己回零到300位置，之后不断读取树莓派的端口状态，当状态改变后改变自己的位置状态，并且当中途夹取到物体（速度小于5）的时候break掉控制循环，接收下一步控制
### 末端的控制通路（连续）
位置---位置
位置差----电流
